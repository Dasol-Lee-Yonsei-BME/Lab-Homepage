...
# Publication years are generated automatically from the data.
...
def img(path, alt=''):
    return f'<img src="{html.escape(str(path))}" alt="{html.escape(str(alt))}">'
...
def head(title, active=''):
    ...
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(title)} · {html.escape(SHORT_NAME)}</title><style>{CSS}</style></head><body><header class="site-header"><div class="header"><a class="brand" href="index.html">{html.escape(SHORT_NAME)}</a><nav class="nav">{nav}</nav></div></header>'''


def foot():
    return f'''<footer class="footer"><div class="footer-inner"><strong>{html.escape(SHORT_NAME)}</strong>{html.escape(str(s['lab_name']))} · {html.escape(str(s['department']))} · {html.escape(str(s['university']))} ({html.escape(str(s['campus']))})</div></footer></body></html>'''


def pagehero(title, text):
    return f'''<section class="page-hero"><div class="page-hero-inner"><div class="eyebrow">{html.escape(SHORT_NAME)} · Yonsei University</div><h1>{title}</h1><p>{text}</p></div></section>'''
...
for p in pubs:
    if p.get('highlight'):
        hl.append(
            ('Publication', p['title'], p['journal'], p['image'])
        )

for p in projs:
    if p.get('highlight'):
        hl.append(
            ('Research Project', p['title'], p['agency'], None)
        )
...
if hl:
    ...
    for kind, title, meta, image in hl[1:3]:
        h += f'''<a class="highlight-small" href="research.html"><div class="kicker">{kind}</div><h3>{title}</h3><p>{meta}</p></a>'''
...
for i, n in enumerate(news_sorted[:6], 1):
    ...
    summary = (
        f'<p>{html.escape(str(n["summary"]))}</p>'
        if n.get('summary')
        else ''
    )
...
h += f'''</div></section><div class="join-strip"><div><h2>Interested in joining {html.escape(SHORT_NAME)}?</h2><p>Graduate students · Undergraduate researchers · Collaborations</p></div><a class="btn" href="join.html">Join {html.escape(SHORT_NAME)} →</a></div></main>'''
...
h = (
    ...
    + pagehero(
        'People',
        f'Principal Investigator, current members, and alumni of {html.escape(SHORT_NAME)}.'
    )
    + '<main><section class="section"><div class="pi">'
    + img(pe['pi']['photo'], pe['pi']['name'])
    + f'''<div><div class="eyebrow">Principal Investigator</div><h2>{pe['pi']['name']}</h2><div class="role">{pe['pi']['title']}</div><p>{pe['pi']['department']}</p><p>{pe['pi']['bio']}</p><p><b>Office</b> {pe['pi']['office']}<br><b>Email</b> {pe['pi']['email']}</p><h3>Education</h3><ul>'''
    + ''.join(
        f'<li>{x}</li>'
        for x in pe['pi']['education']
    )
    + '''</ul></div></div></section><section class="section"><div class="section-head"><div><div class="label">Team</div><h2>Current Members</h2></div></div><div class="member-grid">'''
)
...
for a in pe['alumni']:
    h += f'''<div class="alumni-row"><strong>{a['name']}</strong><span>{a['period']}</span><span>{a['next']}</span></div>'''

h += '</section></main>'
h += foot()

(OUT / 'people.html').write_text(
    h,
    encoding='utf-8'
)
...
def publication_metrics(p):
    ...
    if p.get('impact_factor') not in (None, ''):
        chips.append(
            f'<span class="metric if">IF {html.escape(str(p["impact_factor"]))}</span>'
        )

    if p.get('jcr_percent') not in (None, ''):
        chips.append(
            f'<span class="metric jcr">JCR {html.escape(str(p["jcr_percent"]))}%</span>'
        )
    ...
    if p.get('jcr_category'):
        ...
        if p.get('jcr_year') not in (None, ''):
            label += (
                f' · {html.escape(str(p["jcr_year"]))}'
            )

        chips.append(
            f'<span class="metric category">{label}</span>'
        )
...
for key, title in [
    ...
    for a in ac[key]:
        ...
        if key == 'patents':
            ...
            body = f'''<h3>{a['title']}</h3><p>{a['people']}</p><p>{a['status']} · {a['number']}</p>'''

        elif key == 'awards':

            body = f'''<h3>{a['title']}</h3><p>{a['recipient']} · {a['organization']}</p>'''

        else:

            body = f'''<h3>{a['title']}</h3><p>{a['type']} · {a['event']}</p><p>{a['date']} · {a['location']}</p>'''
...
(OUT / 'achievements.html').write_text(
    h,
    encoding='utf-8'
)
...
news_years = sorted(
    {n['_date_obj'].year for n in news_sorted},
    reverse=True
)
...
for y in news_years:
    ...
    h += (
        f'<section class="news-year-section">'
        f'<h2 class="news-year">{y}</h2>'
        '<div class="news-list">'
    )

    for n in [
        x for x in news_sorted
        if x['_date_obj'].year == y
    ]:

        summary = (
            f'<p class="news-summary">{html.escape(str(n["summary"]))}</p>'
            if n.get('summary')
            else ''
        )

        arrow = (
            f'<a class="news-row-arrow" href="{html.escape(str(n["link"]))}">↗</a>'
            if n.get('link')
